...
router.post("/", protect, async (req, res) => {
    try {
        ...
        if (positions.length === 0)
            return res.status(400).json({ message: "No positions in portfolio" });

        let result;

        if (method === "parametric") {
            result = await computeParametricVaR(positions, confidence, horizonDays);
        } else if (method === "historical") {
            // ⬅️ new addition
            result = await computeHistoricalVaR(positions, confidence, horizonDays);
        } else {
            return res
                .status(400)
                .json({ message: "Unsupported VaR method. Use parametric or historical." });
        }
